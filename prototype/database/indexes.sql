...
CREATE UNIQUE INDEX idx_settings_category_key
...
CREATE INDEX idx_php_versions_default
    ON php_versions (is_default) WHERE is_default = 1;

-- Filter active versions
CREATE INDEX idx_php_versions_active
...
-- Lookup by domain
CREATE INDEX idx_certificates_domain
...
CREATE INDEX idx_certificates_valid_expiry
...
CREATE INDEX idx_plugins_builtin
...
CREATE INDEX idx_config_history_operation
    ON config_history (operation);

-- Combined: entity changes over time
CREATE INDEX idx_config_history_entity_time
    ON config_history (entity_type, entity_id, created_at DESC);

-- Filter by source
CREATE INDEX idx_config_history_source
