 ...
 *  The old split-on-'.' logic parsed "0-beta" as 0 and then compared the
 *  dangling pre-release index as an ordinary numeric part, which ranked
 *  1.0.0-beta.1 AFTER 1.0.0 — inverting semver. Harmless while we only
 *  consume /releases/latest (GitHub excludes pre-releases there) but
 *  trivial to get right so future callers don't inherit the trap. */
