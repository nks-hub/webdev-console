...
public sealed class WindowsFirewallManager
{
    ...
    {
        if (EnvFlags.IsTruthy(Environment.GetEnvironmentVariable("NKS_WDC_SKIP_FIREWALL_UAC")))
        {
            _logger.LogWarning(
                "NKS_WDC_SKIP_FIREWALL_UAC=1 is set - firewall rule registration skipped. " +
