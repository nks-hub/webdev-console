...
completionCmd.SetAction((parseResult, ct) =>
{
    ...
    switch (shell.ToLowerInvariant())
    {
        ...
        case "zsh":
            ...
            AnsiConsole.WriteLine("  local commands=\"new open info config doctor system start-all stop-all restart-all status services logs sites php plugins databases binaries version node compose metrics ssl hosts backup restore migrate uninstall sync\"");
