...
- [~] **Performance monitoring dashboard** — _SiteEdit metrics card + request-rate sparkline + API + CLI done. Historical aggregation beyond the 5-minute client-side window still pending._ `AccessLogInspector` in `Core.Services` inspects candidate Apache access log paths with shared-read FileStream (no lock conflict with Apache). `GET /api/sites/{domain}/metrics` returns `{ domain, hasMetrics, accessLog: { path, sizeBytes, requestCount, lastWriteUtc } }`. `wdc metrics <domain>` CLI shows formatted stats. SiteEdit Metrics tab has 4 metric cards (Total Requests, Access Log Size, Last Request, Requests/min) plus an ECharts line sparkline fed by a client-side ring buffer that polls the metrics endpoint every 5s and computes per-minute request rate from consecutive deltas (60-sample rolling window = 5 minutes). **Still TODO:** server-side historical aggregation beyond raw log tail for longer time windows.
