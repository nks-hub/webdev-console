...
- [~] **Docker Compose integration** — _Detection done (commits `2a92687`, `e72698f`, `b56ed4c`), lifecycle pending._ `DockerComposeDetector` in `Core.Services` scans site document roots for the 4 canonical Compose filenames in v2 priority order. Daemon exposes `GET /api/sites/{domain}/docker-compose`. Sites.vue shows 🐳 Compose badge in the Framework column (auto-refreshes on docroot edit). `wdc compose check <domain>` CLI command with 0/1/2 exit codes for scripting. **Still TODO:** lifecycle control (up/down/logs), per-service listing, integration with daemon's service module.
