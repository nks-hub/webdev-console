...
- [x] **Process supervisor + shutdown lifecycle hardening** — follow-up audit found three real runtime gaps behind the earlier "done" labels: (1) flapping services reset `RestartCount` on every auto-restart, so `MaxRestarts` never tripped; (2) manual stop flowed through the same exit handler as crashes and could emit a false `crashed` state plus restart attempt; (3) `ApplicationStopping.Register(async () => ...)` was fire-and-forget and could exit before cleanup finished. Fixed in the daemon by preserving restart counters across auto-restarts, adding explicit stop intent handling in `ProcessManager`, and blocking the shutdown callback on `StopAllServicesOnShutdownAsync`. Regression coverage added in `ProcessManagerTests`.
...
- [ ] **Self-update mechanism** — updater plumbing is now in place (`electron-updater` wiring in `src/frontend/electron/main.ts`, tray actions for check/install, `electron-builder.json` GitHub publish config, NSIS + portable targets), but the first tagged GitHub release still needs end-to-end validation before this can be marked done.
