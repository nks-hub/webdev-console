...
- [ ] **Self-update mechanism** — updater plumbing is in place (`electron-updater` wiring in `src/frontend/electron/main.ts`, tray actions for check/install, unified `electron-builder.yml`, packaged daemon under `extraResources`, release artifact verifier in `scripts/verify-electron-release.mjs`), but the first tagged GitHub release still needs end-to-end validation before this can be marked done.
