...
**Progress: 7/10 done + 1 partial** — `.php-version` auto-detection, Node.js plugin, Scheduled backups, Site templates, Docker Compose (detection + lifecycle), WebSocket log streaming, **Performance monitoring** partial (AccessLogInspector + metrics API + CLI + SiteEdit tab; charts pending). Nginx, PostgreSQL, RBAC — **not planned** (user decision 2026-04-12).
...
- [—] **Nginx plugin** — _Not planned (user decision 2026-04-12)._
- [—] **PostgreSQL plugin** — _Not planned (user decision 2026-04-12)._
...
- [—] **Multi-user RBAC** — _Not planned (user decision 2026-04-12)._
...
- [x] **Performance monitoring dashboard** — _Fully complete: live + historical._ `AccessLogInspector` in `Core.Services` inspects candidate Apache access log paths with shared-read FileStream (no lock conflict with Apache). `GET /api/sites/{domain}/metrics` returns `{ domain, hasMetrics, accessLog: { path, sizeBytes, requestCount, lastWriteUtc } }`. `wdc metrics <domain>` CLI shows formatted stats. SiteEdit Metrics tab has 4 metric cards (Total Requests, Access Log Size, Last Request, Requests/min) plus an ECharts line sparkline fed by a client-side ring buffer (60 samples × 5s = 5-minute rolling window). Server-side historical aggregation: `MetricsHistoryService` background poller (60s cadence, 7-day retention) writes cumulative request counts to `metrics_history` SQLite table; `GET /api/sites/{domain}/metrics/history?minutes=N&limit=M` returns time-series samples with pre-computed delta-from-previous so frontend can render hour/day/week windows without raw-log re-walks (migration `005_metrics_history.sql`).
