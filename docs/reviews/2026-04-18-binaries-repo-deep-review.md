...
- **[HIGH-B1] ~~icu-config heredoc injection risk~~ — ✅ RESOLVED 2026-04-18 (binaries-repo PR [#21](https://github.com/nks-hub/webdev-console-binaries/pull/21), commit `af9b3b5`, awaits merge).** Shim moved to checked-in `.github/workflows/assets/icu-config-shim.sh` (mode 100755, 33 lines, bash `case "$1"` dispatching pkg-config for all 12 flags probed by PHP `ext/intl/config.m4`). `docker run ... bash -c` 14-line heredoc replaced with single `cp /work/.github/workflows/assets/icu-config-shim.sh /usr/local/bin/icu-config`. Added `actions/checkout@v4` as first step (workflow previously had zero checkout — `${{ github.workspace }}` bind-mount was empty). Quote-escape injection vector closed; `$VAR` expansion from host runner env no longer can leak into guest container shim. `bash -n` syntax check + `yaml.safe_load` pass.
...
- **[MEDIUM-B5] ~~skip_source empty matrix slot no marker~~ — ✅ RESOLVED 2026-04-18 (binaries-repo PR [#16](https://github.com/nks-hub/webdev-console-binaries/pull/16), commit `069dd86`, stacked on PR #3).** Parallel upload step uploads `SKIPPED-<target>-<version>.txt` marker to every skipped slot (gated by `matrix.target != 'windows-x64' && env.skip_source == 'true'`). Marker text cites the reason (PECL/autoconf/libxml2/ICU toolchain drift for PHP <7.1), alternatives (Windows prebuilt, local build, upgrade), and workflow run URL + commit SHA for traceability. Net effect: `gh release view binaries-php-7.0.33` now shows 1 Windows zip + 2 SKIPPED markers instead of lone Windows zip.
...
- **[MEDIUM-C3] ~~mariadb bintar-systemd silent fallback~~ — ✅ RESOLVED 2026-04-18 (binaries-repo PR [#20](https://github.com/nks-hub/webdev-console-binaries/pull/20), commit `8288567`, stacked on PR #13).** `::notice::mariadb ${VER} linux source: systemd-bintar (…)` / `… plain-bintar (systemd fallback)` logged per successful curl branch. Downstream debugging now has explicit audit trail for which tarball shipped (matters for musl/distroless incompat).
...
- **[MEDIUM-E1] ~~mkcert mirror 3 triplets vs catalog 5~~ — ✅ RESOLVED 2026-04-18 (binaries-repo PR [#17](https://github.com/nks-hub/webdev-console-binaries/pull/17), commit `de2bf13`, awaits merge).** `build-mkcert.yml` extended to full 5-triplet coverage: added `linux-arm64` (`mkcert-v${V}-linux-arm64`) + `macos-x64` (`mkcert-v${V}-darwin-amd64`). Both URLs verified 200 on FiloSottile/mkcert releases. `chmod +x` list extended. **Catalog flip follow-up:** after PR #17 merges + next `binaries-mkcert-*` tag run publishes the 5 assets, `wdc-catalog-api/app/data/apps/mkcert.json` should swap FiloSottile URLs → nks-hub mirror URLs (JSON snippet documented in PR body).

### F. `build-nginx.yml` (129 lines)

- **[MEDIUM-F1] ~~nginx macOS arm64 dynamically linked to brew paths~~ — ✅ RESOLVED 2026-04-18 (binaries-repo PR [#18](https://github.com/nks-hub/webdev-console-binaries/pull/18), commit `694c16b`, awaits merge).** Option A (dylib bundling) applied: new step between `make install` and `Package` copies pcre2/openssl@3/zlib `.dylib` from brew into `$STAGE/lib/`, rewrites `sbin/nginx` + bundled dylib `install_name_tool -change` references to `@executable_path/../lib/`, rewrites each dylib's `LC_ID_DYLIB`, hard-fails if any absolute `/opt/homebrew` or `/usr/local` ref remains. macOS-only guard (`matrix.target == 'macos-arm64'`). Tarball grows ~387 KB → ~5–10 MB. Code signing = future work (needs Apple developer cert). Options:
...
- **[MEDIUM-G1] ~~redis fork URL selection silent~~ — ✅ RESOLVED 2026-04-18 (binaries-repo PR [#20](https://github.com/nks-hub/webdev-console-binaries/pull/20), commit `8288567`).** `PICKED_URL` captured inside `if curl` branch; post-loop `case` emits `::notice::` with fork name (tporadowski/redis OR redis-windows fallback). Zero-match path upgraded from silent-broken-`ls` to explicit `::error::` + `exit 1`. Audit trail now captures which fork built each Windows zip.
