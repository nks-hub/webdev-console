...
5. **[HIGH NEW] ~~0-bit of upstream verification~~ — ✅ PARTIALLY RESOLVED 2026-04-18 (binaries-repo PR [#4](https://github.com/nks-hub/webdev-console-binaries/pull/4), commit `0d94c39`, awaits merge).** PHP workflow (`build-binaries.yml`) gets SHA256 verification on all three upstream fetches: Windows (`windows.php.net/sha256sum.txt`), Linux legacy (hash lookup on host runner before `ubuntu:20.04` container), Linux modern + macOS (inline verify after curl). Canonical hash source is **PHP.net JSON API** `releases/?json&version=X.Y.Z` (the `.sha256` sidecar URL cited in roadmap returns 404 — plan deviation documented in PR). GPG `.asc` files left as TODO → SECURITY.md roadmap item #2. **Apache, MariaDB, nginx, redis workflows still unverified** — follow-up PRs one per workflow.
