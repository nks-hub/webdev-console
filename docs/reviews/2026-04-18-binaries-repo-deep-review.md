...
1. **[CRITICAL NEW] `httpd-2.4.66-windows-x64.zip` is a 2,451-byte HTML error page, not a ZIP.** The catalog currently points `wdc binaries install apache@2.4.66` on Windows at this asset. Every Windows install of Apache 2.4.66 via WDC silently fetches HTML and crashes during extraction. Root cause: `build-apache.yml:65-70` scrapes the ApacheLounge VS17 index, discovers `httpd-2.4.66-NNNNNN-win64-VS17.zip`, then `curl -fsSL`'s the `/binaries/` URL. When ApacheLounge rotates that stale build off the server, the `/binaries/` URL 200-OKs with HTML, not 404. `-fsSL` doesn't check content-type. No post-download size/magic check. The workflow reported "success" and uploaded HTML-as-ZIP. Evidence: `curl -sIL https://www.apachelounge.com/download/VS17/binaries/httpd-2.4.66-250207-win64-VS17.zip` → `HTTP 200`, `Content-Length: 2451`, `Content-Type: text/html`. Contents: `<html><head><link rel="stylesheet" href="style.css">...<TITLE>Apache Lounge</TITLE>...`.
2. **[CRITICAL] GitHub's native per-asset `digest: sha256:...` is exposed by the REST API but neither catalog nor daemon consumes it.** Every `gh release view` response returns `assets[].digest = "sha256:<hex>"` — computed by GitHub at upload, immutable thereafter. 22 tags × ~3 assets = 61 free sha256 hashes already sitting in the API response. `wdc-catalog-api` emits them as `null` on all 61 download rows. `CatalogClient.DownloadDoc` doesn't carry a `sha_256` field. One catalog-generator commit + one daemon-downloader commit closes the entire "no integrity verification" finding from the prior audit without touching a single workflow YAML.
3. **[CRITICAL] `main` branch has zero protection; every workflow and `dependabot.yml` change lands via direct push.** `gh api repos/.../branches/main/protection` → `HTTP 404 Branch not protected`. Repo-level toggles: `secret_scanning: disabled`, `secret_scanning_push_protection: disabled`, `dependabot_security_updates: disabled`. There is no `CODEOWNERS` file (`gh api repos/.../contents/.github/CODEOWNERS` → 404). Any maintainer account compromise or leaked PAT can edit `build-binaries.yml` and push tag `binaries-php-8.3.25-r2` and repoison every PHP line in a single CI run — no review gate, no branch protection, no secret scanning, no CODEOWNERS.
4. **[HIGH NEW] `fail_on_unmatched_files` boolean-coerces a string expression and is effectively always-false for PHP.** `build-binaries.yml:451` → `fail_on_unmatched_files: ${{ env.skip_source != 'true' || matrix.target == 'windows-x64' }}`. `softprops/action-gh-release@v2` treats unquoted inputs as strings; the expression evaluates to the strings `"true"` or `"false"` per-matrix-slot, which the action then parses. This **works by accident** but is a timebomb: any future rename of `env.skip_source`, or the action tightening its parser, silently reverts to "don't fail on missing files" — which is how the 2026-04-17 PHP 7.x `success` results were declared even when Linux/macOS legs partially produced zero files (see run history: multiple `"ci: per-matrix-target file pattern fixes fail_on_unmatched regression"` retries in 10 min). The current success state is fragile.
5. **[HIGH NEW] 0-bit of upstream verification, even where upstream publishes `.sig`/`.sha256` *beside* the tarball.** Prior audit already flagged this; deeper observation: the workflow `curl`s happen on the *same line* as the tarball — adding a verify step is 2 lines. Concrete upstream hash/sig availability per workflow:
...
- **[HIGH-A3] Package tarball includes runtime dirs like `logs`.** `build-apache.yml:156-157` conditionally tars `logs htdocs icons man manual cgi-bin`. `logs` is empty-but-present; `htdocs` is the stock welcome page; `manual` adds ~4 MB. `logs` in particular is a Linux-FHS runtime dir — shipping it pre-created with the installer is fine, but never flush `access_log` before tarring (no explicit check for leftover artifacts from the CI run; `make install` shouldn't write logs but a broken test would).
...
- **[HIGH-B2] PECL tgz URLs are `curl`-fetched per build with zero integrity check and no `--fail-with-body`.** `build-binaries.yml:382` `curl -fsSL -o ext.tgz https://pecl.php.net/get/${name}-${ver}.tgz`. PECL has been known to serve stale/cached empty responses during outages. `-fsSL` silences the server error body. Same HTML-as-tgz risk as the Apache case. A poisoned PECL tgz runs `phpize` + `configure` + `make install` as root inside the workspace, then publishes the compromised `.so` under the tag.
- **[HIGH-B3] `install_pecl_ext` silently `|| echo "::warning::"` on failure.** Line 392. A PECL build failure logs a warning but the workflow exits 0 and the release ships *without* that extension. Catalog does not declare per-extension coverage, so a silent drop of e.g. Redis ext or APCu isn't detectable downstream. Minimum fix: aggregate warnings into a manifest file uploaded alongside the archive, or fail the job on any warning when matrix.target is `linux-x64` (the "reference" Linux build).
- **[MEDIUM-B4] 15 of 39 PHP workflow runs failed in the last 50** (from run history re-query: 24 success / 38 success-total vs 24 failure in recent 80). Failures cluster around the `-r2`/`-r3` retry-suffix pattern, suggesting repeated tag pushes to force rebuilds. With no `concurrency:` block, **two simultaneous dispatches** (happens during bulk rebuild) race at the `softprops/action-gh-release` upload step — last write wins. Silent partial-release risk.
...
- **[HIGH-C1] `fail_on_unmatched_files: false` + three `curl ... || true`.** Lines 56, 60, and 62 each `|| true`. The MSI and macOS tar.gz are *allowed* to fail silently. `fail_on_unmatched_files` is explicitly false. So: if archive.mariadb.org removes the OSX arm64 tarball (as it has for many historical versions) the workflow still ships a Linux-only release, with no indication that macOS was intended. Explains exactly why 11.8.3 and 12.3.1 are Windows+Linux-only — `bintar-osx_arm64/` isn't published for those versions and the `|| true` swallows the 404.
- **[HIGH-C2] MSI vs ZIP format mismatch.** Prior audit (H5). Catalog advertises `.zip` for Windows mariadb; workflow produces `.msi`. Daemon's `BinaryDownloader` has no MSI handler. Re-confirmed: `gh release view binaries-mariadb-12.3.1 --json assets` → `mariadb-12.3.1-windows-x64.msi` (92 MB). The catalog's Windows URL still points at `archive.mariadb.org` directly (zip), so nks-hub MSI is *doubly* orphaned: wrong format for daemon, not referenced by catalog.
...
- **[HIGH-D1] `cloudflared-${VER}-macos-x64` is uploaded as an **extracted bare binary** (40 MB) while `cloudflared-${VER}-macos-arm64.tgz` ships **as-is** (20 MB).** Lines 76-79 `tar xzf darwin-amd64.tgz && mv cloudflared ...-macos-x64 && rm darwin-amd64.tgz` — amd64 is explicitly extracted. arm64 uses `|| true` after the curl: if download fails the extract step never fires, so the `-macos-arm64.tgz` stays on disk *from a prior iteration* or never appears at all. In the actual `binaries-cloudflared-2026.3.0` release both exist: `cloudflared-2026.3.0-macos-arm64` (37 MB, extracted) AND `cloudflared-2026.3.0-macos-arm64.tgz` (20 MB, raw). Evidence of a past run's `cloudflared-darwin-arm64.tgz` lingering in the workspace + getting uploaded. Two near-identical assets. Catalog entries for `cloudflared` macos-arm64 point at the bare binary; the `.tgz` is pure dead weight.
...
**[HIGH-RB1] SECURITY.md makes a provenance claim the releases do not fulfill.** "Each release is ... auditable from the release page → 'Built from' link → workflow run." There is no such link on any release body. This is a documentation/reality divergence.
